...
class QRNNLayer(nn.Module):
    ...
    Args:
        input_size: The number of expected features in the input x. кол-во признаков
        hidden_size: The number of features in the hidden state h. If not specified, the input size is used.
        save_prev_x: Whether to store previous inputs for use in future convolutional windows (i.e. for a continuing sequence such as in language modeling). If true, you must call reset to remove cached previous values of x. Default: False.
        window: Defines the size of the convolutional window (how many previous tokens to look when computing the QRNN values). Supports 1 and 2. Default: 1.
        zoneout: Whether to apply zoneout (i.e. failing to update elements in the hidden state) to the hidden state updates. Default: 0. обновлять ли эл-ты в скрытом состоянии
        output_gate: If True, performs QRNN-fo (applying an output gate to the output). If False, performs QRNN-f. Default: True. - QRNN-fo. Если false-QRNN-f
...
#Создаётся список QRNNLayer. Если 2 слоя, то второй принимает на вход hidden_size первого.
